...
import com.wfc.cloudsim.cloudsim.lists.PeList;
...
/**
 * PodScheduler is an abstract class that represents the policy used by a Virtual Machine Monitor (VMM)
 * to share processing power of a PM among VMs running in a host. 
 * 
 * Each host has to use is own instance of a PodScheduler
...
public abstract class PodScheduler {
	...
	/** The map of VMs to MIPS, were each key is a VM id and each value is 
         ...
         * in the {@link #peMap}.
         * 
         * @todo subclasses such as {@link PodSchedulerTimeShared} have an
         * {@link PodSchedulerTimeShared#mipsMapRequested} attribute that
	...
	/**
	 * Creates a new PodScheduler.
	 * 
	 * @param pelist the list of PEs of the host where the PodScheduler is associated to.
	 * @pre peList != $null
	 * @post $none
	 */
	 ...
	 * @post $none
	 */
	public abstract boolean allocatePesForVm(Pod pod, List<Double> mipsShare);
	 ...
	 * @post $none
	 */
	public abstract void deallocatePesForVm(Pod pod);

	/**
	 * Releases PEs allocated to all the VMs of the host the PodScheduler is associated to.
	...
	/**
	 * Gets the pes allocated for a pod.
	 * 
	 * @param pod the pod
	 * @return the pes allocated for the given pod
	 */
	 ...
	 * 
	 * @param pod the pod
	 * @return the total allocated mips for the pod
	 */
	public double getTotalAllocatedMipsForVm(Pod pod) {
		double allocated = 0;
		List<Double> mipsMap = getAllocatedMipsForVm(pod);
		if (mipsMap != null) {
	 ...
	 * @return mips
         ...
         * received by the PodScheduler can be heterogeneous PEs.
	 */
