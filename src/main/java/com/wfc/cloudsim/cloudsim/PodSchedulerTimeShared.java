...
import com.wfc.cloudsim.cloudsim.provisioners.PeProvisioner;

import java.util.*;

/**
 * PodSchedulerTimeShared is a Virtual Machine Monitor (VMM) allocation policy that allocates one or more PEs
...
public class PodSchedulerTimeShared extends PodScheduler {
	 ...
	 */
	public PodSchedulerTimeShared(List<? extends Pe> pelist) {
		super(pelist);
		setMipsMapRequested(new HashMap<String, List<Double>>());
	}

	@Override
	public boolean allocatePesForVm(Pod pod, List<Double> mipsShareRequested) {
		/*
		 * @todo add the same to RAM and BW provisioners
		 */
		if (pod.isInMigration()) {
			if (!getVmsMigratingIn().contains(pod.getUid()) && !getVmsMigratingOut().contains(pod.getUid())) {
				getVmsMigratingOut().add(pod.getUid());
			}
		} else {
			if (getVmsMigratingOut().contains(pod.getUid())) {
				getVmsMigratingOut().remove(pod.getUid());
			}
		}
		boolean result = allocatePesForVm(pod.getUid(), mipsShareRequested);
	...
	public void deallocatePesForVm(Pod pod) {
		getMipsMapRequested().remove(pod.getUid());
		setPesInUse(0);
		getMipsMap().clear();
		setAvailableMips(PeList.getTotalMips(getPeList()));

		for (Pe pe : getPeList()) {
			pe.getPeProvisioner().deallocateMipsForVm(pod);
		}
