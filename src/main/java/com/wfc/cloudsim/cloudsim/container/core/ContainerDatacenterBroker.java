...
import com.wfc.cloudsim.cloudsim.UtilizationModelPlanetLabInMemory;
    ...
    protected void createVmsInDatacenter(int datacenterId) {
        ...
        for (ContainerPod vm : getVmList()) {
            if (!getVmsToDatacentersMap().containsKey(vm.getId())) {
                Log.printLine(String.format("%s: %s: Trying to aaaCreate VM #%d in %s", CloudSim.clock(), getName(), vm.getId(), datacenterName));
...
//                Log.print("User has fcom.wfc.cloudsimotten to bound the cloudlet to container");
