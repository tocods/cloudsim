...
import com.wfc.cloudsim.cloudsim.PodStateHistoryEntry;
import com.wfc.cloudsim.cloudsim.container.containerProvisioners.ContainerBwProvisioner;
import com.wfc.cloudsim.cloudsim.container.containerProvisioners.ContainerPe;
import com.wfc.cloudsim.cloudsim.container.containerProvisioners.ContainerRamProvisioner;
import com.wfc.cloudsim.cloudsim.container.lists.ContainerPeList;
import com.wfc.cloudsim.cloudsim.container.schedulers.ContainerScheduler;
...
/**
 * Pod represents a VM: it runs inside a Host, sharing hostList with other VMs. It processes
...
//        Log.printLine("Pod: update Vms Processing at " + currentTime);
        if (mipsShare != null && !getContainerList().isEmpty()) {
            double smallerTime = Double.MAX_VALUE;
            Log.printLine("ContainerPod " + getId() + " : update Vms Processing");
            Log.printLine("The container list size is:...." + getContainerList().size());

            for (Container container : getContainerList()) {
                List<Double> doubles = new ArrayList<>();
                for(int i = 0; i < container.getNumberOfPes(); i++) {
                    doubles.add(mipsShare.get(0));
                }
                //double time = container.updateContainerProcessing(currentTime, getContainerScheduler().getAllocatedMipsForContainer(container));
                double time = container.updateContainerProcessing(currentTime, doubles);
                if (time > 0.0 && time < smallerTime) {
                    smallerTime = time;
                }
            }
//            Log.printLine("ContainerPod: The Smaller time is:......" + smallerTime);
...
//        }
    ...
    public List<Double> getCurrentRequestedMips() {
        ...
        //Log.printLine("Pod: get Current requested Mips" + currentRequestedMips);
    ...
    public long getCurrentRequestedBw() {
        ...
        if (isBeingInstantiated()) {
            ...
            //Log.printLine("Pod: get Current requested Mips" + requestedBwTemp);
    ...
    public float getCurrentRequestedRam() {
        if (isBeingInstantiated()) {
            ...
            //Log.printLine("Pod: get Current requested Mips" + requestedRamTemp);
    ...
    public double getTotalUtilizationOfCpu(double time) {
        ...
        //Log.printLine("Pod: get Current requested Mips" + TotalUtilizationOfCpu);
...
//        Log.printLine("ContainerPod: addMigratingInContainer:......");
    ...
    public double updateContainersProcessing(double currentTime) {
        double smallerTime = Double.MAX_VALUE;
//        Log.printLine("ContainerPod: update Vms Processing");
//        Log.printLine("The VM list size is:...." + getContainerList().size());
        ...
        //Log.printLine("Pod: The Smaller time is:......" + smallerTime);
    ...
    public void removeMigratingInContainer(Container container) {
        ...
        Log.printLine("ContainerPod# "+getId()+"removeMigratingInContainer:......" + container.getId() + "   Is deleted from the list");
...
//        Log.printLine("ContainerPod: re alocating MigratingInContainer:......");
    ...
    public void containerDestroy(Container container) {
        //Log.printLine("Pod:  Destroy Container:.... " + container.getId());
        if (container != null) {
            containerDeallocate(container);
//            Log.printConcatLine("The Container To remove is :   ", container.getId(), "Size before removing is ", getContainerList().size(), "  vm ID is: ", getId());
            getContainerList().remove(container);
            Log.printLine("ContainerPod# "+getId()+" containerDestroy:......" + container.getId() + "Is deleted from the list");
...
//        Log.printLine("ContainerPod: Destroy all Containers");
        containerDeallocateAll();
        for (Container container : getContainerList()) {
            container.setVm(null);
            setSize(getSize() + container.getSize());
        }
//        Log.printLine("ContainerPod# "+getId()+" : containerDestroyAll:...... the whole list is cleared ");
...
//        Log.printLine("ContainerPod: Deallocated the VM:......" + vm.getId());
    ...
    /**
     * Deallocate all podList for the container.
     */
    protected void containerDeallocateAll() {
//        Log.printLine("ContainerPod: Deallocate all the Vms......");
...
//        Log.printLine("ContainerPod: get the PeList Size......" + getPeList().size());
...
//        Log.printLine("ContainerPod: get the free Pes......" + ContainerPeList.getNumberOfFreePes(getPeList()));
...
//        Log.printLine("ContainerPod: get the total mips......" + ContainerPeList.getTotalMips(getPeList()));
    ...
    public boolean allocatePesForContainer(Container container, List<Double> mipsShare) {
        //Log.printLine("ContainerPod: allocate Pes for Container:......" + container.getId());
    ...
    public void deallocatePesForContainer(Container container) {
        //Log.printLine("ContainerPod: deallocate Pes for Container:......" + container.getId());
        getContainerScheduler().deallocatePesForContainer(container);
    }
    ...
    public double getTotalAllocatedMipsForContainer(Container container) {
        //Log.printLine("ContainerPod: total allocated Pes for Container:......" + container.getId());
    ...
    public double getMaxAvailableMips() {
        //Log.printLine("ContainerPod: Maximum Available Pes:......");
    ...
    public double getAvailableMips() {
        //Log.printLine("ContainerPod: Get available Mips");
     ...
     */
    public long getBw() {
        //Log.printLine("ContainerPod: Get BW:......" + getContainerBwProvisioner().getBw());
     ...
     */
    public float getRam() {
        //Log.printLine("ContainerPod: Get Ram:......" + getContainerRamProvisioner().getRam());
...
//        Log.printLine("Host: Create VM???......" + container.getId());
        WFCConstants.containerMips.put(container.getId(), getMips());
        if (getSize() < container.getSize()) {
        ...
        Log.printLine("Pod " + getId() + " give Container " + container.getId() + " mips: " + getMips());
        container.changeMips(getMips());
