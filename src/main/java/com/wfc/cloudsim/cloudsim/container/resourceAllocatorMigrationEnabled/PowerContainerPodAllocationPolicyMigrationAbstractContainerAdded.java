...
import com.wfc.cloudsim.cloudsim.container.lists.*;
import com.wfc.cloudsim.cloudsim.container.podSelectionPolicies.PowerContainerPodSelectionPolicy;
import com.wfc.cloudsim.cloudsim.container.resourceAllocators.PowerContainerAllocationPolicy;
...
public abstract class PowerContainerPodAllocationPolicyMigrationAbstractContainerAdded extends PowerContainerPodAllocationPolicyMigrationAbstract {
    ...
    public PowerContainerPodAllocationPolicyMigrationAbstractContainerAdded(List<? extends ContainerHost> hostList,
    ...
    private List<Map<String, Object>> getNewContainerPlacement(List<? extends Container> containersToMigrate, Set<? extends ContainerHost> excludedHosts) {
        ...
        for (Container container : containersToMigrate) {
            ...
            if (allocationMap.get("host") != null && allocationMap.get("vm") != null) {
                ContainerPod vm = (ContainerPod) allocationMap.get("vm");
...
//                   We try to create the largest Pod possible
                List<ContainerPod> VmList = createVms(host, true);
                if(VmList.size() != 0){
                    for(ContainerPod vm:VmList){
...
//                    vm.setInWaiting(true);
    ...
    protected ContainerPod createVMinHost(ContainerHost host, boolean vmStatus) {

        for (int i=0; i<numberOfVmTypes; i++) {
            ContainerPod vm = getNewVm(i);
...
//                        previouseVm.setInWaiting(true);
    ...
    protected Map<String, Object> findAvailableHostForContainer(Container
        ...
        ContainerPod allocatedVm = null;
        List<ContainerHost> underUtilizedHostList = new ArrayList<>();
        List<ContainerPod> vmList = new ArrayList<>();
        ...
        for (ContainerHost host1 : underUtilizedHostList) {
            ...
            for(Map<String, Object> map:createdVm){
                ...
                vmList.add((ContainerPod) map.get("vm"));
                }
...
//                if vm is not created no need for checking!
    ...
    private ContainerPod getNewVm(int vmType) {
...
//        Log.print(vmType);
        ...
        int brokerId = 2;
        PowerContainerPod vm = new PowerContainerPod(IDs.pollId(ContainerPod.class), brokerId, (double) vmMips[vmType],
                vmRam[vmType],
                vmBw, vmSize, "Xen",
    ...
    protected boolean isVmOverUtilized(ContainerPod vm) {
...
//        Log.printConcatLine("Checking if the vm is over utilized or not!");
     ...
     */
    protected double getPowerAfterContainerAllocation(PowerContainerHost host, Container container, ContainerPod vm) {
        double power = 0;
        try {
     ...
     * @return the power after allocation
     */
    protected double getMaxUtilizationAfterContainerAllocation(PowerContainerHost host, Container container, ContainerPod containerPod) {
        double requestedTotalMips = container.getCurrentRequestedTotalMips();
        if (requestedTotalMips > containerPod.getMips()) {
            requestedTotalMips = containerPod.getMips();
        }
     ...
     *
     * @param containerPod the host
     * @return the utilization of the CPU in MIPS
     */
    protected double getUtilizationOfCpuMipsofVm(ContainerPod containerPod) {
        double vmUtilizationMips = 0;
        for (Container container : containerPod.getContainerList()) {

            vmUtilizationMips += containerPod.getTotalAllocatedMipsForContainer(container);
        }
     ...
     */
    protected boolean isHostOverUtilizedAfterContainerAllocation(PowerContainerHost host, ContainerPod vm, Container container) {
    ...
    protected void restoreAllocation() {
        ...
        for (Map<String, Object> map : getSavedAllocation()) {
            PowerContainerPod vm = (PowerContainerPod) map.get("vm");
...
//        List<ContainerPod > restoredVms = new ArrayList<>();
        for (Map<String, Object> map : getSavedAllocation()) {
            PowerContainerPod vm = (PowerContainerPod) map.get("vm");
...
//            container.setVm(vm);
    ...
    protected List<ContainerPod> createVms(ContainerHost host, boolean vmStatus) {
        List<ContainerPod> vmList = new ArrayList<>();
        while (true) {
            ContainerPod vm = createVMinHost(host, vmStatus);
            if (vm == null) {
                break;
            }
            vmList.add(vm);
