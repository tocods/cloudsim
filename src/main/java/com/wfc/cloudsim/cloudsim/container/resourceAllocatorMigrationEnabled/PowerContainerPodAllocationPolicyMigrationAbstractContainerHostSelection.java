...
import com.wfc.cloudsim.cloudsim.container.lists.PowerContainerList;
 ...
 */
public abstract class PowerContainerPodAllocationPolicyMigrationAbstractContainerHostSelection extends PowerContainerPodAllocationPolicyMigrationAbstractContainerAdded {

    private HostSelectionPolicy hostSelectionPolicy;

    public PowerContainerPodAllocationPolicyMigrationAbstractContainerHostSelection(List<? extends ContainerHost> hostList, PowerContainerPodSelectionPolicy vmSelectionPolicy,
    ...
    public Map<String, Object> findHostForContainer(Container container, Set<? extends ContainerHost> excludedHosts, boolean checkForVM) {
        ...
        while (true) {
            ...
            List<ContainerPod> vmList = host.getVmList();
            PowerContainerPodList.sortByCpuUtilization(vmList);
            for (int i = 0; i < vmList.size(); i++) {
                ContainerPod vm = vmList.get(vmList.size() - 1 - i);
                if(checkForVM){
                    if(vm.isInWaiting()){
    ...
    protected List<Map<String, Object>> getNewContainerPlacementFromUnderUtilizedHost(
        ...
        for (Container container : containersToMigrate) {
            ...
            if (allocatedMap.get("vm") != null && allocatedMap.get("host")!= null) {
                ...
                Log.printConcatLine("Container# ",container.getId(),"allocated to VM # ", ((ContainerPod)allocatedMap.get("vm")).getId()
    ...
    while (true) {
        ...
        for(Map<String, Object> map2:createdVm){
            if(map2.get("host")== host){
                vmList.add((ContainerPod) map2.get("vm"));
            }
