 ...
 */
public class ContainerPodAllocationPolicySimple extends ContainerPodAllocationPolicy {
     ...
     * @post $none
     */
    public ContainerPodAllocationPolicySimple(List<? extends ContainerHost> list) {
        super(list);
    ...
    @Override
    public boolean allocateHostForPods(List<ContainerPod> vm){
        return false;
    }
    ...
    public boolean allocateHostForVm(ContainerPod containerPod) {
        int requiredPes = containerPod.getNumberOfPes();
        ...
        if (!getVmTable().containsKey(containerPod.getUid())) { // if this vm was not created
            do {// we still trying until we find a host or until we try all of them
                ...
                result = host.containerVmCreate(containerPod);

                if (result) { // if vm were succesfully created in the host
                    getVmTable().put(containerPod.getUid(), host);
                    getUsedPes().put(containerPod.getUid(), requiredPes);
    ...
    public void deallocateHostForVm(ContainerPod containerPod) {
        ...
        int idx = getContainerHostList().indexOf(host);
        int pes = getUsedPes().remove(containerPod.getUid());
        if (host != null) {
    ...
    @Override
    public ContainerHost getHost(ContainerPod containerPod) {
        return getVmTable().get(containerPod.getUid());
    }
