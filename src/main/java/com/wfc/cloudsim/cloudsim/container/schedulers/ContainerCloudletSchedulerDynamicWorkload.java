...
import com.wfc.cloudsim.cloudsim.ResCloudlet;
...
public class ContainerCloudletSchedulerDynamicWorkload extends ContainerCloudletSchedulerTimeShared {
         ...
         */
        public ContainerCloudletSchedulerDynamicWorkload (int id, double mips, int numberOfPes) {
            super();
            this.conId = id;
            setMips(mips);
        ...
        public double updateContainerProcessing(double currentTime, List<Double> mipsShare) {
            Log.printLine("Container is updating processing, mipsShare is " + mipsShare);
            ...
            for (ResCloudlet rcl : getCloudletExecList()) {
                ...
                rcl.updateCloudletFinishedSoFar((long) (timeSpan
                        * mipsShare.get(0) * mipsShare.size() * Consts.MILLION));
                if (rcl.getRemainingCloudletLength() == 0) { // finished: remove from the list
                    ...
                    double estimatedFinishTime = getEstimatedFinishTime(rcl, currentTime, mipsShare);
        ...
        public double cloudletSubmit(Cloudlet cl, double fileTransferTime) {
            ...
            return getEstimatedFinishTime(rcl, getPreviousTime(), getCurrentMipsShare());
        }
        ...
        public List<Double> getCurrentRequestedMips() {
            ...
            mipsForPe = WFCConstants.containerMips.get(conId);
        ...
        public double getTotalCurrentRequestedMipsForCloudlet(ResCloudlet rcl, double time) {
            return rcl.getCloudlet().getUtilizationOfCpu(time) * getNumberOfPes() * WFCConstants.containerMips.get(conId);
        }
         ...
         */
        public double getEstimatedFinishTime(ResCloudlet rcl, double time, List<Double> shares) {
            return time
                    + ((rcl.getRemainingCloudletLength()) / getTotalCurrentAllocatedMipsForCloudlet(rcl, time));
            /*return time
                    + ((rcl.getRemainingCloudletLength()) / shares.size() * shares.get(0));*/
        }
