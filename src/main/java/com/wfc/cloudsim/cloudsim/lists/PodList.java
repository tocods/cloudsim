...
package com.wfc.cloudsim.cloudsim.lists;
...
/**
 * PodList is a collection of operations on lists of VMs.
 * 
...
public class PodList {
	...
	/**
	 * Gets a {@link Pod} with a given id.
	 * 
	 * @param id ID of required VM
	 * @param vmList list of existing VMs
	 * @return a Pod with the given ID or $null if not found
	 * @pre $none
	 * @post $none
         * 
	 ...
	 */
	public static <T extends Pod> T getById(List<T> vmList, int id) {
		for (T vm : vmList) {
			if (vm.getId() == id) {
				return vm;
			}
	...
	/**
	 * Gets a {@link Pod} with a given id and owned by a given user.
	 * 
	 * @param vmList list of existing VMs
	 * @param id ID of required VM
	 ...
	 * @post $none
	 */
	public static <T extends Pod> T getByIdAndUserId(List<T> vmList, int id, int userId) {
		for (T vm : vmList) {
