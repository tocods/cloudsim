 ...
 */
public abstract class PowerPodAllocationPolicyMigrationAbstract extends PowerPodAllocationPolicyAbstract {

	/** The vm selection policy. */
	private PowerPodSelectionPolicy vmSelectionPolicy;

	/** A list of maps between a VM and the host where it is place.
         * @todo This list of map is implemented in the worst way.
         * It should be used just a Map<Pod, Host> to find out
	...
	/**
	 * Instantiates a new PowerPodAllocationPolicyMigrationAbstract.
	 * 
	 * @param hostList the host list
	 * @param vmSelectionPolicy the vm selection policy
	 */
	public PowerPodAllocationPolicyMigrationAbstract(
	...
	public List<Map<String, Object>> optimizeAllocation(List<? extends Pod> vmList) {
		...
		List<? extends Pod> vmsToMigrate = getVmsToMigrateFromHosts(overUtilizedHosts);
	...
	protected List<Map<String, Object>> getMigrationMapFromUnderUtilizedHosts(
		...
		while (true) {
			...
			List<? extends Pod> vmsToMigrateFromUnderUtilizedHost = getVmsToMigrateFromUnderUtilizedHost(underUtilizedHost);
			...
			if (!Log.isDisabled()) {
				for (Pod pod : vmsToMigrateFromUnderUtilizedHost) {
					Log.print(pod.getId() + " ");
				}
			}
			Log.printLine();
	 ...
	 */
	public PowerHost findHostForVm(Pod pod, Set<? extends Host> excludedHosts) {
		...
		for (PowerHost host : this.<PowerHost> getHostList()) {
			...
			if (host.isSuitableForVm(pod)) {
				if (getUtilizationOfCpuMips(host) != 0 && isHostOverUtilizedAfterAllocation(host, pod)) {
					continue;
				}

				try {
					double powerAfterAllocation = getPowerAfterAllocation(host, pod);
	 ...
	 * @param pod the candidate pod
	 * @return true, if the host will be over utilized after VM placement; false otherwise
	 */
	protected boolean isHostOverUtilizedAfterAllocation(PowerHost host, Pod pod) {
		boolean isHostOverUtilizedAfterAllocation = true;
		if (host.vmCreate(pod)) {
	...
	public PowerHost findHostForVm(Pod pod) {
		...
		if (pod.getHost() != null) {
			excludedHosts.add(pod.getHost());
		}
		return findHostForVm(pod, excludedHosts);
	}
	...
	protected List<Map<String, Object>> getNewVmPlacement(
			List<? extends Pod> vmsToMigrate,
			Set<? extends Host> excludedHosts) {
		List<Map<String, Object>> migrationMap = new LinkedList<Map<String, Object>>();
		PowerPodList.sortByCpuUtilization(vmsToMigrate);
		for (Pod pod : vmsToMigrate) {
			PowerHost allocatedHost = findHostForVm(pod, excludedHosts);
			if (allocatedHost != null) {
				allocatedHost.vmCreate(pod);
				Log.printConcatLine("VM #", pod.getId(), " allocated to host #", allocatedHost.getId());
	...
	protected List<Map<String, Object>> getNewVmPlacementFromUnderUtilizedHost(
		...
		for (Pod pod : vmsToMigrate) {
			...
			if (allocatedHost != null) {
				...
				for (Map<String, Object> map : migrationMap) {
					((Host) map.get("host")).vmDestroy((Pod) map.get("pod"));
				}
				migrationMap.clear();
				break;
			}
	 ...
	 */
	protected List<? extends Pod>
	  getVmsToMigrateFromHosts(List<PowerHostUtilizationHistory> overUtilizedHosts) {
		List<Pod> vmsToMigrate = new LinkedList<Pod>();
		for (PowerHostUtilizationHistory host : overUtilizedHosts) {
			while (true) {
				Pod pod = getVmSelectionPolicy().getVmToMigrate(host);
				if (pod == null) {
					break;
				}
				vmsToMigrate.add(pod);
				host.vmDestroy(pod);
	 ...
	 */
	protected List<? extends Pod> getVmsToMigrateFromUnderUtilizedHost(PowerHost host) {
		List<Pod> vmsToMigrate = new LinkedList<Pod>();
	...
	protected void saveAllocation() {
		...
		for (Host host : getHostList()) {
			for (Pod pod : host.getVmList()) {
				if (host.getVmsMigratingIn().contains(pod)) {
					continue;
				}
	...
	protected void restoreAllocation() {
		...
		for (Map<String, Object> map : getSavedAllocation()) {
			Pod pod = (Pod) map.get("pod");
			PowerHost host = (PowerHost) map.get("host");
			if (!host.vmCreate(pod)) {
				Log.printConcatLine("Couldn't restore VM #", pod.getId(), " on host #", host.getId());
				System.exit(0);
			}
	 ...
	 */
	protected double getPowerAfterAllocation(PowerHost host, Pod pod) {
		double power = 0;
		try {
			power = host.getPowerModel().getPower(getMaxUtilizationAfterAllocation(host, pod));
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(0);
		}
	 ...
	 * @return the power after allocation
	 */
	protected double getMaxUtilizationAfterAllocation(PowerHost host, Pod pod) {
		double requestedTotalMips = pod.getCurrentRequestedTotalMips();
	...
	protected double getUtilizationOfCpuMips(PowerHost host) {
		...
		for (Pod pod2 : host.getVmList()) {
			if (host.getVmsMigratingIn().contains(pod2)) {
				// calculate additional potential CPU usage of a migrating in VM
				hostUtilizationMips += host.getTotalAllocatedMipsForVm(pod2) * 0.9 / 0.1;
			}
			hostUtilizationMips += host.getTotalAllocatedMipsForVm(pod2);
		}
	 ...
	 */
	protected void setVmSelectionPolicy(PowerPodSelectionPolicy vmSelectionPolicy) {
	 ...
	 */
	protected PowerPodSelectionPolicy getVmSelectionPolicy() {
