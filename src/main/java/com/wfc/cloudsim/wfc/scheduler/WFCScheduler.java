package com.wfc.cloudsim.wfc.scheduler;

import com.wfc.cloudsim.cloudsim.container.core.*;
import com.wfc.cloudsim.cloudsim.container.lists.ContainerList;
import com.wfc.cloudsim.cloudsim.container.lists.ContainerPodList;
...
import com.wfc.cloudsim.cloudsim.lists.CloudletList;
...
import com.wfc.cloudsim.workflowsim.failure.FailureGenerator;
...
import com.wfc.cloudsim.workflowsim.scheduling.BaseSchedulingAlgorithm;
import com.wfc.cloudsim.workflowsim.scheduling.DataAwareSchedulingAlgorithm;
import com.wfc.cloudsim.workflowsim.scheduling.FCFSSchedulingAlgorithm;
import com.wfc.cloudsim.workflowsim.scheduling.MCTSchedulingAlgorithm;
import com.wfc.cloudsim.workflowsim.scheduling.MaxMinSchedulingAlgorithm;
import com.wfc.cloudsim.workflowsim.scheduling.MinMinSchedulingAlgorithm;
import com.wfc.cloudsim.workflowsim.scheduling.RoundRobinSchedulingAlgorithm;
import com.wfc.cloudsim.workflowsim.scheduling.StaticSchedulingAlgorithm;
...
//        Log.printConcatLine("The Pod ID is ",  CloudletList.getById(getCloudletList(), cloudletId).getVmId(), "should be", vmId);
    ...
    public void setTaskLists(SimEvent ev) {

        List<Task> tasks = (List<Task>) ev.getData();
        //Log.printLine("Scheduler: receive " + tasks.size() + " tasks");
        this.tasks = tasks;
    }
    ...
    public void processContainerCreate(SimEvent ev) {
        ...
        int result = data[2];
        Log.printLine("Scheduler: pod: " + vmId + " container: " + containerId);
        if (result == CloudSimTags.TRUE) {
            if(vmId ==-1){
                Log.printConcatLine("Error : Where is the VM (VM Id = -1)");}
            else{
            getContainersToVmsMap().put(containerId, vmId);
            ContainerList.getById(getContainerList(), containerId).changeMips(ContainerList.getById(getContainerList(), containerId).getVm().getMips());
...
//            ContainerPod p= ContainerPodList.getById(getVmsCreatedList(), vmId);
            ...
            Log.printLine("Container created: " + getContainersCreated());
        } else {
...
//                getDatacenterIdsList().size(), " resource(s)");
    ...
    protected void processNewVmCreate(SimEvent ev) {
        ...
        if (result == CloudSimTags.TRUE) {
            containerPod.setMips(containerPod.getHost().getContainerVmScheduler().getOneMips());
    ...
    protected void processVmCreate(SimEvent ev) {
        ...
        if (result == CloudSimTags.TRUE) {
            getVmsToDatacentersMap().put(vmId, datacenterId);
            ContainerPodList.getById(getVmList(), vmId).setMips(ContainerPodList.getById(getVmList(), vmId).getHost().getContainerVmScheduler().getOneMips());
...
//        If we have tried creating all of the vms in the data center, we submit the containers.
    ...
    protected void processCloudletReturn(SimEvent ev) {
        Log.printLine("Scheduler return");
    ...
    protected void createVmsInDatacenter(int datacenterId) {
        ...
        List<ContainerPod> pods = new ArrayList<>();
        for (ContainerPod vm : getVmList()) {
            if (!getVmsToDatacentersMap().containsKey(vm.getId())) {
                Log.printLine(String.format("%s: %s: Trying to Create Pod #%d in %s", CloudSim.clock(), getName(), vm.getId(), datacenterName));
                //sendNow(datacenterId, CloudSimTags.VM_CREATE_ACK, vm);
                pods.add(vm);
                requestedVms++;
            }
        }
        sendNow(datacenterId, CloudSimTags.VM_CREATE_ACK, pods);
...
//------------------------------------------------
     ...
     *
     */
    private BasePlanningAlgorithm getPlanningAlgorithm(Parameters.PlanningAlgorithm name) {
    ...
    protected void processCloudletUpdate(SimEvent ev) {
        ...
        //Log.printLine("Scheduler: cloudlet's size: " + getCloudletList().size());
        //Log.printLine("if it's job: " + getCloudletList().get(0).ifJob());
        ...
        {
            ifPlan = true;
            planner = getPlanningAlgorithm(Parameters.getPlanningAlgorithm());
            planner.setTaskList(scheduledList);
            planner.setVmList(getVmsCreatedList());
        }
        if(ifPlan && scheduledList.size() < WFCConstants.WFC_JOB_NUMBER) {
        ...
        for (ContainerCloudlet cloudlet : scheduledList) {
            if (containerIndex < getContainersCreated()) {
                //Log.printLine("1: " + cloudlet.getCloudletId() + " " + cloudlet.getVmId() + " " + cloudlet.getContainerId());
                if(getContainersToVmsMap().get(cloudlet.getContainerId()) != null) {
                    //Log.printLine("2");
                    int vmId = getContainersToVmsMap().get(cloudlet.getContainerId());
                    if(cloudlet.ifJob()) {
                        //Log.printLine("scheduler ===> job: " + cloudlet.getCloudletId() + cloudlet.getContainerId());
                    } else {
                    ...
                    // int containerId= getContainersToVmsMap().get(cloudlet.getContainerId());
                     ...
                     //Log.printLine("Scheduler: schedule " + getVmsToDatacentersMap().get(vmId));
    ...
    protected void processCloudletSubmit(SimEvent ev) {
        Log.printLine("Scheduler submit");
