...
import com.wfc.cloudsim.wfc.scheduler.WFCScheduler;
import com.wfc.cloudsim.workflowsim.reclustering.ReclusteringEngine;
...
public final class WFCEngine extends SimEntity {
    ...
    public WFCEngine(String name) throws Exception {
        this(name, 1, new ArrayList<>(), new ArrayList<>());
    }

    public WFCEngine(String name, int schedulers, List<ContainerPod> pods, List<Container> containers) throws Exception {
        super(name);
        ...
        for (int i = 0; i < schedulers; i++) {
           ...
           // vmList = createVmList(wfs.getId(),  WFCConstants.WFC_NUMBER_VMS);
            vmList = new ArrayList<>(pods);
            for(int j = 0; j < vmList.size(); j++) {
                vmList.get(j).setUserId(wfs.getId());
            }
            ...
            for(int j = 0; j < containerList.size(); j++) {
                containerList.get(j).setUserId(wfs.getId());
            }
